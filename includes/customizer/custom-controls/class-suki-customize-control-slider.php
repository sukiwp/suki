...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Suki_Customize_Control_Slider' ) ) {
	...
	class Suki_Customize_Control_Slider extends WP_Customize_Control {
		...
		public function to_json() {
			...
			$this->json['hideUnits'] = 1 === $this->hide_units ? true : false;
		...
		protected function content_template() {
			...
			<div class="customize-control-content">
				<# _.each( data.responsiveStructures, function( setting_key, device ) { #>
					<div class="suki-slider-fieldset suki-flex-fieldset {{ data.hasResponsive ? 'suki-responsive-fieldset' : '' }} {{ 'desktop' == device ? 'active' : '' }} {{ 'preview-' + device }}">
						<input class="suki-slider-range" type="range" value="{{ data.settingsData[ setting_key ].number }}" min="{{ data.units[ data.settingsData[ setting_key ].unit ].min }}" max="{{ data.units[ data.settingsData[ setting_key ].unit ].max }}" step="{{ data.units[ data.settingsData[ setting_key ].unit ].step }}">
						<div class="suki-dimension">
							<input class="suki-slider-number suki-dimension-number" type="number" value="{{ data.settingsData[ setting_key ].number }}" min="{{ data.units[ data.settingsData[ setting_key ].unit ].min }}" max="{{ data.units[ data.settingsData[ setting_key ].unit ].max }}" step="{{ data.units[ data.settingsData[ setting_key ].unit ].step }}">
