					...
					<li class="suki-widget-post-item">
						<a href="<?php the_permalink( $qpost->ID ); ?>">
							...
							if ( has_post_thumbnail( $qpost->ID ) ) {
								echo get_the_post_thumbnail( $qpost->ID, apply_filters( 'suki_widget_posts_thumbnail_size', 'thumbnail', $default ) );
							}
...
// register
function register_suki_widget_posts() {
	register_widget( 'Suki_Widget_Posts' );
}
add_action( 'widgets_init', 'register_suki_widget_posts' );