...
class Suki_Customizer {
	...
	protected function __construct() {
		...
		// Customizer CSS rebuilding triggers
		add_action( 'after_switch_theme', array( $this, 'write_static_css' ) );
		add_action( 'customize_save_after', array( $this, 'write_static_css' ) );
		add_action( 'update_option_suki_enable_customizer_static_css', array( $this, 'write_static_css' ) );
		add_action( 'suki_version_changed', array( $this, 'write_static_css' ) );

		add_action( 'suki_compatible_plugin_activated', array( $this, 'compatible_plugin_rebuild_customizer_css' ) );
		add_action( 'suki_compatible_plugin_deactivated', array( $this, 'compatible_plugin_rebuild_customizer_css' ) );

		// Use parent customizer settings mode
		// ref:https://core.trac.wordpress.org/ticket/27177#comment:4
		if ( is_child_theme() && apply_filters( 'suki_use_parent_theme_mods', true ) ) {
	...
	public function register_settings( $wp_customize ) {
		...
		// Global Settings
		require_once( SUKI_INCLUDES_PATH . '/customizer/options/settings--scripts.php' );
		...
		// Disable contextual hiding for Widgets
		add_action( 'wp', function() use ( $wp_customize ) {
			foreach ( array( 'sidebar', 'sidebar-shop', 'footer-widgets-1', 'footer-widgets-2', 'footer-widgets-3', 'footer-widgets-4', 'footer-widgets-5', 'footer-widgets-6' ) as $slug ) {
				$section = $wp_customize->get_section( 'sidebar-widgets-' . $slug );

				if ( ! empty( $section ) ) {
					$section->active_callback = '__return_true';
				}
			}
		});
	}
	...
	/**
	 * Write generated customizer CSS into a static CSS file.
	 */
	public function write_static_css() {
		// Create CSS folder on wp-uploads.
		$wp_upload_dir = wp_upload_dir( null, false );
		$folder = $wp_upload_dir['basedir'] . '/suki';
		...
		// Header info.
		$header = esc_html( '/* Generated by Suki v' . suki_get_theme_info( 'version' ) . ' at ' . current_time( 'mysql' ) . ' */' . "\n" );
	...
	/**
	 * Rebuild Customizer CSS file if plugin data has 'rebuild_customizer_css' as true.
	 *
	 * @param array $plugin_data
	 */
	public function compatible_plugin_rebuild_customizer_css( $plugin_data ) {
		// Rebuilding Customizer CSS file.
		if ( $plugin_data['rebuild_customizer_css'] ) {
